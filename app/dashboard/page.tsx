...
const SAMPLE_MEETINGS = [
  { id:'1', title:'Team Weekly Sync', date:'اليوم', time:'10:00 ص', duration:'45 د', color:'#4361EE',
    transcript:'صباح الخير الجميع. نبدأ بتحديث السبرينت — وصلنا ٨٥٪ من sprint 12. المراجعة التصميمية الخميس الساعة ٢م.',
    summary:{ overview:'اجتماع أسبوعي: تقدم السبرينت ومراجعة التصميم والمهام.', keyPoints:['Sprint 12 عند ٨٥٪','مراجعة التصميم الخميس ٢م','تأهيل موظف جديد الأسبوع القادم'], actions:['تحديث تذاكر Jira','إرسال الأصول للفريق التقني'], next:['متابعة العائق مع الفريق الخلفي'] }},
  { id:'2', title:'عرض Acme Corp', date:'اليوم', time:'٢:٣٠ م', duration:'٣٠ د', color:'#059669',
    transcript:'شكراً لانضمامكم. سنستعرض اليوم أحدث تحسينات لوحة التحكم. العميل أعجبه الواجهة الجديدة.',
    summary:{ overview:'عرض Acme Corp: إعجاب بالواجهة وموافقة على المرحلة الثانية.', keyPoints:['إعجاب بالواجهة الجديدة','طلب تصدير مخصص','الميزانية موافق عليها'], actions:['إرسال الاقتراح','جدولة اجتماع تقني'], next:['الاقتراح قبل الجمعة'] }},
  { id:'3', title:'خارطة طريق Q3', date:'أمس', time:'١١:٠٠ ص', duration:'٦٠ د', color:'#7B2FBE',
    transcript:'نبدأ بالتخطيط لـ Q3. أولويتنا الأولى ميزات الذكاء الاصطناعي. تطبيق الجوال مؤجل للـ Q4.',
    summary:{ overview:'تخطيط Q3: تقديم AI وتأجيل الجوال.', keyPoints:['AI أولوية Q3','الجوال للـ Q4','نموذج تسعير جديد'], actions:['إنهاء المواصفات','تحديث خارطة الطريق'], next:['مراجعة الاثنين'] }},
];
...
export default function Dashboard() {
  ...
  const startRec = async () => {
    ...
    txRef.current = ''; setTimer(0); setTitle('اجتماع جديد'); setChatMsgs([]); setPanel('summary');
  ...
  const stopRec = async () => {
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: 'user', content: `Analyze this meeting transcript and return ONLY:\n{"overview":"2-3 sentences","keyPoints":["p1","p2"],"actions":["a1"],"next":["n1"]}\nTranscript: ${tx}` }]
        })
      });
    ...
    } catch(e) { setSummary({ overview:'تعذّر الاتصال بـ Claude.', keyPoints:[], actions:[], next:[] }); }
  ...
  const sendChat = async () => {
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 600, system: `أنت مساعد اجتماعات. أجب بالعربية بشكل موجز بناءً على المحضر:\n${tx}`, messages: [{ role: 'user', content: q }] })
      });
      const d = await res.json();
    ...
    <div dir="rtl" style={{display:'flex',height:'100vh',background:'#F0F4FF',fontFamily:'"Plus Jakarta Sans",system-ui,sans-serif',color:'#0D1117',overflow:'hidden'}}>
      ...
      <div style={{width:260,flexShrink:0,background:'#fff',borderLeft:'1px solid #EEF2FF',display:'flex',flexDirection:'column',boxShadow:'2px 0 20px rgba(67,97,238,.06)'}}>
        ...
        <div style={{padding:'20px 16px 16px',borderBottom:'1px solid #F0F4FF'}}>
          ...
          <div style={{position:'relative'}}>
            <button onClick={()=>setShowProfile(v=>!v)} style={{width:'100%',display:'flex',alignItems:'center',gap:10,padding:'10px 12px',borderRadius:12,background:'#F8FAFF',border:'1.5px solid #EEF2FF',cursor:'pointer',fontFamily:'inherit',textAlign:'right'}}>
              <div style={{width:34,height:34,borderRadius:10,background:'linear-gradient(135deg,#4361EE,#7B2FBE)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,fontWeight:800,color:'#fff',flexShrink:0}}>
            ...
            {showProfile&&(
              <div style={{position:'absolute',top:'100%',right:0,left:0,background:'#fff',border:'1.5px solid #EEF2FF',borderRadius:12,marginTop:4,boxShadow:'0 8px 24px rgba(67,97,238,.1)',zIndex:100,overflow:'hidden',animation:'fadeUp .15s ease'}}>
                ...
                <button onClick={async()=>{await supabase.auth.signOut();window.location.href='/login';}} style={{width:'100%',display:'flex',alignItems:'center',gap:9,padding:'10px 14px',background:'none',border:'none',cursor:'pointer',fontFamily:'inherit',fontSize:12.5,color:'#EF4444',textAlign:'right',borderRadius:0}}>
                  🚪 تسجيل الخروج
                </button>
              </div>
            )}
        ...
        <div style={{padding:'14px'}}>
          <button className="hbtn" onClick={isRec?stopRec:startRec} disabled={status==='processing'} style={{width:'100%',padding:'12px',borderRadius:12,border:'none',cursor:'pointer',background:isRec?'linear-gradient(135deg,#EF4444,#B91C1C)':'linear-gradient(135deg,#4361EE,#7B2FBE)',color:'#fff',fontWeight:700,fontSize:13.5,display:'flex',alignItems:'center',justifyContent:'center',gap:8,fontFamily:'inherit',boxShadow:isRec?'0 4px 16px rgba(239,68,68,.35)':'0 4px 16px rgba(67,97,238,.35)',transition:'all .2s'}}>
            {isRec
              ?<><div style={{width:10,height:10,background:'#fff',borderRadius:2}}/>إيقاف وتحليل</>
              :<><div style={{width:8,height:8,background:'#fff',borderRadius:'50%'}}/>تسجيل جديد</>
            }
          </button>
        </div>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}} onClick={()=>showProfile&&setShowProfile(false)}>
        ...
        <div style={{padding:'14px 24px',background:'#fff',borderBottom:'1px solid #EEF2FF',display:'flex',alignItems:'center',gap:12,flexShrink:0}}>
          <div style={{flex:1,minWidth:0}}>
            {isRec
              ?<input autoFocus value={title} onChange={e=>setTitle(e.target.value)} placeholder="عنوان الاجتماع..." style={{fontSize:16,fontWeight:700,color:'#0D1117',background:'transparent',border:'none',outline:'none',fontFamily:'inherit',letterSpacing:'-.3px',width:'100%'}}/>
          ...
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            {isRec&&(
              <div style={{display:'flex',alignItems:'center',gap:8,background:'#FFF0F0',padding:'6px 14px',borderRadius:20,border:'1px solid #FFD0D0'}}>
        ...
        {/* Waveform */}
        {isRec&&(
          <div style={{padding:'16px 24px',background:'linear-gradient(135deg,#EEF2FF,#F5EEFF)',borderBottom:'1px solid #E8EDF8',display:'flex',alignItems:'center',justifyContent:'center',gap:10,flexShrink:0,animation:'fadeUp .3s ease'}}>
            <div style={{display:'flex',alignItems:'center',gap:2.5,height:56}}>
              {wave.map((h,i)=><div key={i} style={{width:3,height:`${h}px`,borderRadius:3,transition:'height .08s ease',background:`linear-gradient(to top,#4361EE,#7B2FBE)`}}/>)}
            </div>
        ...
        {/* Processing */}
        {status==='processing'&&(
          <div style={{padding:'12px',background:'#FFFBEB',borderBottom:'1px solid #FEF3C7',display:'flex',alignItems:'center',justifyContent:'center',gap:8,flexShrink:0}}>
        ...
        <div style={{flex:1,display:'flex',overflow:'hidden'}}>
          ...
          {/* IDLE */}
          {isIdle&&(
            <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:32,padding:32,animation:'fadeUp .4s ease'}}>
              ...
              <div style={{textAlign:'center'}}>
                ...
                <p style={{fontSize:14,color:'#6B7FA3',lineHeight:1.75}}>iMeets يسجّل · يُحوّل للنص · يُلخّص بـ Claude AI</p>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12}}>
                {[{icon:'🎙',t:'تسجيل',d:'صوت واضح'},{icon:'📝',t:'نسخ فوري',d:'بالعربية'},{icon:'✦',t:'Claude AI',d:'ملخص ذكي'},{icon:'💬',t:'AI Chat',d:'اسأل وجاوب'}].map(f=>(
                  <div key={f.t} style={{padding:'16px',borderRadius:14,background:'#fff',border:'1.5px solid #EEF2FF',textAlign:'center',boxShadow:'0 2px 10px rgba(67,97,238,.05)'}}>
                    <div style={{fontSize:22,marginBottom:8}}>{f.icon}</div>
                    <div style={{fontSize:12,fontWeight:700,color:'#0D1117',marginBottom:3}}>{f.t}</div>
                    <div style={{fontSize:10.5,color:'#9AAAC8'}}>{f.d}</div>
                  </div>
                ))}
              </div>
                  ...
                  <>
                    ...
                    <div style={{flex:1,overflow:'hidden'}}>
                      ...
                      {summary&&<div style={{padding:'18px',overflowY:'auto',height:'100%'}}>
                        ...
                        {[{k:'keyPoints',l:'النقاط الرئيسية',c:'#4361EE',bg:'#EEF2FF',i:'◆'},{k:'actions',l:'مهام العمل',c:'#7B2FBE',bg:'#F5EEFF',i:'⚡'},{k:'next',l:'الخطوات التالية',c:'#059669',bg:'#ECFDF5',i:'→'}].map(s=>summary[s.k]?.length>0&&(
        ...
        {/* Bottom bar */}
        {!isIdle&&(
          <div style={{padding:'10px 24px',borderTop:'1px solid #EEF2FF',display:'flex',alignItems:'center',gap:9,background:'#fff',flexShrink:0}}>
